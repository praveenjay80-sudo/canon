...
- `src/components/PulseView.jsx` — 4 plain ranked-list panels (Most Cited, Rising, Most Influential Papers, Google Scholar), shared `Panel` renderer, mono metric chips, no prose; Google Scholar's empty state is `ScholarKeyPrompt` (inline key input) when no `canon_serp_key` is set, otherwise a plain "no results" message
