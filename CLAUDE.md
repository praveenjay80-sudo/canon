...
9. **Pulse** (`appMode === 'pulse'`) — the one Claude-free mode: live citation data, no LLM synthesis at all. Input is 3 cascading dropdowns (Field → Subfield → Topic) via `fetchOpenAlexTaxonomy()`, not free text — selecting a leaf Topic (real OpenAlex topic ID) fires parallel live fetches: OpenAlex works filtered by exact topic ID (Most Cited + Rising, the latter re-sorted client-side by 2-year citation velocity from `counts_by_year`), Semantic Scholar cross-referenced by the DOIs of those same OpenAlex works via a batch lookup (Most Influential Papers — precise, not a separate fuzzy search), and Google Scholar as the 4th tier via the `canon-enrichment` Worker's `/scholar-search` (see Cloudflare Worker section — never call `serpapi.com` directly from the browser, it has no CORS headers). A user's own `canon_serp_key` is passed through and takes priority over the worker's shared key; if the fetch fails (no key anywhere, bad key, or the shared key is quota-limited), the panel shows an inline key-entry prompt instead of an empty state, and saving a key calls `refreshScholar()` to backfill just that panel without re-running the other fetches. Open Syllabus Project was dropped from Pulse (its API now 403s even keyless — see Open Syllabus note below) — all 4 panels are plain ranked lists, title/authors/year + a mono metric chip, no prose
...
- `src/hooks/usePulse.js` — no Claude call. On topic selection: OpenAlex works filtered by exact topic id (`pulseOpenAlex.js`) in parallel with a Google Scholar search via the `canon-enrichment` Worker's `/scholar-search` (passes `canon_serp_key` through if set); then a Semantic Scholar batch DOI lookup against the OpenAlex results' DOIs for the "Most Influential" cross-reference. Tracks `scholarFailed` (true on any fetch/worker error, not just "no key") and exports `refreshScholar()`, called after a key is saved from Pulse's own inline prompt, to backfill just the Scholar panel. Phases `idle → loading → complete/error`, no "harvesting"/"generating" split since there's no LLM stage
...
- `src/components/PulseView.jsx` — 4 plain ranked-list panels (Most Cited, Rising, Most Influential Papers, Google Scholar), shared `Panel` renderer, mono metric chips, no prose; Google Scholar's empty state is `ScholarKeyPrompt` (inline key input) when `scholarFailed`, otherwise a plain "no results" message
